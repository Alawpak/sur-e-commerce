...
// Drop a replacement at the same path — no code changes needed. If missing,
// the section still works: solid bg, no broken image.
const HERO_CUTOUT = "/hero/girl_hero.png";
...
export function ProductHeroV2() {
  ...
  return (
    <section className="relative min-h-[100svh] w-full overflow-hidden bg-bone">
      ...
      <div
        className="pointer-events-none absolute inset-0 z-0 flex items-start justify-center overflow-hidden pt-24 sm:pt-28 lg:items-center lg:justify-start lg:pt-0"
        aria-hidden
      >
        <span
      ...
      {src ? (
        <div className="absolute inset-0 z-0 flex items-end justify-center lg:justify-end lg:pr-[8%] xl:pr-[12%]">
          ...
          <img
            ...
            className="h-[85%] w-auto max-w-none object-contain sm:h-[100%] lg:h-[130%]"
          />
        </div>
      ) : null}

      {/* Bottom-left: XOXO SUR */}
      <div className="absolute bottom-8 left-6 z-10 flex items-end gap-2.5 sm:bottom-10 sm:gap-3">
        <span
          className="font-display font-black leading-[0.85] tracking-tight text-ink"
      ...
      <div className="absolute bottom-8 right-6 z-10 flex max-w-[160px] flex-col gap-3 text-right sm:max-w-xs sm:bottom-10 sm:gap-4 md:max-w-sm">
        ...
        <Link
          href="/shop"
          className="text-[11px] tracking-ultra text-ink underline underline-offset-4 hover:opacity-60"
        >
          SHOP NOW
        </Link>
      </div>
